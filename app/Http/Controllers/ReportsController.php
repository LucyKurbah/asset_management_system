...
class ReportsController extends Controller
{
    ...
    {   
        ...
        return view('modules/reports/employee_report')->with(compact('role','employee'));
    }
    ...
    public function GetReport(Request $request)
    {
        ...
        $allocatedDevices = Allocation::with('employee', 'assignedTo','device.categoryRelation','device.itemType','device.oemRelation','device')
        ->where('allocation.emp_code', $employeeCode)
        ->get();

        // dd($allocatedDevices);
     
         return response()->json($allocatedDevices)->withHeaders([
            'Cache-Control' => 'max-age=15, public',
            'Expires' => gmdate('D, d M Y H:i:s', time() + 15) . ' IST',
        ]);
    }
   
    public function printEmpRegister(Request $request, $emp_code)
    {

        $allocatedDevices = Allocation::with('employee', 'assignedTo','device.categoryRelation','device.itemType','device.oemRelation','device')
                        ->where('allocation.emp_code', $emp_code)
                        ->get();

        foreach ($allocatedDevices as $device) {
            $device->issued_on = \Carbon\Carbon::parse($device->issued_on);
            // $device->returned_on = \Carbon\Carbon::parse($device->returned_on);
            if (!empty($device->returned_on)) {
                $device->returned_on = Carbon::parse($device->returned_on);
            } else {
                // Handle the case where the date is empty or null
                // For instance, setting it to null or a default value
                $device->returned_on = null; // Or set a default date: $device->returned_on = Carbon::now();
            }
        }

      // dd($allocatedDevices);
        $pdf = PDF::loadView('reports.employee_register', compact('allocatedDevices'));

        return $pdf->stream('employee_register.pdf',array('Attachment'=>0));
    }
    ...
    {
        $advertisements = Advertisement::with(['assigned_news.empanelled.news_type', 'subject'])->get();
    ...
    public function printBillingRegister(Request $request, $from, $to)
    {
        ...
        $bills = Bill::select('b.id','a.hod','e.news_name','a.release_order_no','a.release_order_date','b.bill_no','b.bill_date','a.size','a.amount','b.paid_by')
                        ->from('bills as b')
