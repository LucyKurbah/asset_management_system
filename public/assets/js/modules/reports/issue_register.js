$(document).ready(function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        console.log(response);
          ...
          $.each(response, function (indexInArray, advertisement) { 
            ...
            var positively_on = formatDate(advertisement.positively);
            ...
            table += '<tr>'+
               ...
               '<td class="text-center">'+ advertisement.size+ '</td>'+
               '<td class="text-center">'+ advertisement.subject.subject_name+ '</td>'+
               '<td class="text-center">'+ advertisement.ref_no + ' Dt. '+ ref_date +'</td>'+
               '<td class="text-center">'+ advertisement.positively_on + '</td>'+
               '<td class="text-center">'+ advertisement.no_of_entries + '</td>'+
               '<td class="text-center" >'+ newspaperNames+ '</td>'+
      ...
      $(document).on('click', '.btn-get-report', function () {
        $('.loader-overlay').show();
    ...
    function populateIssueRegisterTable(response,from,to) {
        ...
        $.each(response, function (indexInArray, advertisement) { 
          ...
          var issueDate = formatDate(advertisement.issue_date);
          var ref_date = formatDate(advertisement.ref_date);
          var positively_on = formatDate(advertisement.positively_on);
          if(advertisement.remarks == null)
              remarks='';
          else
              remarks = advertisement.remarks;
            var newRow = $('<tr>').append(
                ...
                $('<td class="text-center">').text(advertisement.size),
                $('<td class="text-center">').text(advertisement.subject.subject_name),
                $('<td class="text-center">').text(advertisement.ref_no + ' Dt. ' + ref_date),
                $('<td class="text-center">').text(positively_on),
                $('<td class="text-center">').text(advertisement.no_of_entries),
