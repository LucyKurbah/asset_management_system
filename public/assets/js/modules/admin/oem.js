    ...
    $.ajax({
      ...
      success: function (response) {
          ...
          $.each(response, function (indexInArray, valueOfElement) { 
               ...
               '<td class="text-center">'+ valueOfElement.oem_name+ '</td>';
               table += '<td class="text-center"><span style="color:darkblue;" data-id="'+ valueOfElement.id+'"class="icon ri-edit-2-fill oem-edit"></span> &nbsp; &nbsp;<span style="color:red;" data-id="'+ valueOfElement.id+'" class="icon ri-chat-delete-fill oem-delete"></span> </td>'
               '</tr>';
          });
      ...
      $(document).on('click','.oem-edit',function (e) { 
          e.preventDefault();
          var id = $(this).data('id');
          var value;
          $.ajax({
            ...
            success:function(data){
              $('#oem-form').trigger("reset");
              $("#id").val(data[0]['id']);
              $("#oem").val(data[0]['oem_name']);
              $("#users-tab").tab('show');
            }
          });
        });
  
        $(document).on('click','.oem-delete',function (e) { 
          e.preventDefault();
          var id = $(this).data('id');
          $.ajax({
            type: "POST",
