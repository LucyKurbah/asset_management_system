    ...
    $.ajax({
      ...
      success: function (response) {
          ...
          $.each(response, function (indexInArray, valueOfElement) { 
               ...
               '<td class="text-center">'+ valueOfElement.item_name+ '</td>';

               
               table += '<td class="text-center"><span style="color:darkblue;" data-id="'+ valueOfElement.id+'"class="icon ri-edit-2-fill item_type_edit"></span> &nbsp; &nbsp;<span style="color:red;" data-id="'+ valueOfElement.id+'" class="icon ri-chat-delete-fill item_type_delete"></span> </td>'
               '</tr>';
          });
      ...
      $('#item_type_form').on('submit', function(e) {
          e.preventDefault();
          var formData = new FormData(this);
          $.ajax({
            type:'POST',
            url:'/item_type-store-data',
            data: formData,
            cache:false,
            contentType: false,
            processData: false,
        ...
        $(document).on('click','.item_type_edit',function (e) { 
          e.preventDefault();
          var id = $(this).data('id');
          ...
          $.ajax({
            ...
            success:function(data){
              ...
              $('#item_type_form').trigger("reset");
              $("#id").val(data[0]['id']);
              $("#item_type").val(data[0]['item_name']);
              $("#users-tab").tab('show');
            }
          });
        });
  
        $(document).on('click','.item_type_delete',function (e) { 
          e.preventDefault();
          var id = $(this).data('id');

          $.ajax({
            type: "POST",
            url: "/item_type_delete",
            data: {id},
            cache:false,
            success:function(data){
              if(data['flag']=='Y'){
