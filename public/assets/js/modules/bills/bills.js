$(document).ready(function () {
      ...
      success: function (response) {
        console.log(response);
          ...
          $.each(response, function (indexInArray, bill) { 
            ...
            if(bill.paid_by== 'O')
            {
              f= '<a href="/reports/forwarding_letter/' + bill.id + '" style="color: black; font-size: 20px;" data-id="' + bill.id + '" class="icon ri-download-fill release_order" target="_blank"></a>';
            }
            else
              f='';
              ...
              var billDate = formatDate(bill.bill_date);
               ...
               table += '<td class="text-center"><span style="color:darkblue;" data-id="'+ bill.id+'"class="icon ri-edit-2-fill bill-edit"></span> &nbsp; &nbsp;<span style="color:red;font-size:20px;" data-id="'+ bill.id+'" class="icon ri-close-line bill-delete"></span> &nbsp; &nbsp;  </td>'+
    ...
    function getNewspaper()
    {
      ...
      $.ajax({
        ...
        success: function (response) {
          $('#empanelled_id').empty();
          $('#empanelled_id').append('<option value="" disabled selected>--Select newspaper--</option>');
          // Populate options with news_name from empanelled relationship
          $.each(response.assigned_news, function (index, news) {
              $('#empanelled_id').append('<option value="' + news.empanelled.id + '">' + news.empanelled.news_name + '</option>');
          });
      ...
      $.ajax({
        ...
        success: function (response) {
          if(response != '')
          {
            Swal.fire({
              icon: 'warning',
              title: 'Bill already exist with bill no: '+ response[0]['bill_no'],
    ...
    $('#bill-form').on('submit', function(e) {
          ...
          $.ajax({
            ...
            success:function(data){
              console.log(data);
              if(data['flag']=='Y' || data['flag']=='YY'){ //Succcess for creation of new user
              {
                  if(data['flag']=='Y')
                  {
                    $(".table_msg1").show();
              ...
              else if(data['flag']=='NN'){ //Error while editing an Ad
                $(".table_msg6").show();
          ...
          $.ajax({
            ...
            success:function(data){
              ...
              $("#bill_date").val(formatDateFromDB(data[0]['bill_date']));
               ...
               $("#paid_by option[value='" + data[0]['paid_by'] + "']").prop('selected', true);
              $("#users-tab").tab('show');
            }
          });
    });
