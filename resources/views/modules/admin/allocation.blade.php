            ...
            <div class="card-body">
              ...
              <ul class="nav nav-tabs nav-tabs-bordered d-flex" id="myTab" role="tablist">
                <li class="nav-item flex-fill" role="presentation">
                  <button class="nav-link w-100 active" id="user-tab" data-bs-toggle="tab" data-bs-target="#bordered-justified-food" type="button" role="tab" aria-controls="home" aria-selected="true"><i class="bi bi-card-list"></i> 
                  &nbsp;Allocation</button>
                </li>
                <li class="nav-item flex-fill" role="presentation">
                  <button class="nav-link w-100" id="users-tab" data-bs-toggle="tab" data-bs-target="#bordered-justified-foods" type="button" role="tab" aria-controls="profile" aria-selected="false"><i class="ri-file-add-line"></i>  &nbsp; Add/Edit Allocation</button>
                </li>
              </ul>
              ...
              <div class="modal fade" id="disablebackdrop" tabindex="-1" data-bs-backdrop="false">
                ...
                <form id="allocation-form" method="POST" >
                  ...
                  <input type="hidden" id="id" name="id">
                        ...
                        <div class="col-md-6">
                            <label for="emp_code" class="form-label"><b> Employee</b></label>
                            ...
                            <select name="item_type" id="item_type" class="form-control">
                                @if($item_type)
                                  <option value=''>--Select an Item Type--</option>
                                    @foreach($item_type as $dev)                                 
                                    <option value="{{ $dev->id }}">
                                      {{ $dev->item_name  }} 
                                    </option>
                                    @endforeach
                                @else
                                  <option value=''>--No Item Type for allocation--</option>
                                @endif
                            </select>
                            ...
                            <select name="device" id="device" class="form-control">
                                @if($device)
                                  <option value=''>--Select a Device--</option>
                                    ...
                                    <option value="{{ $dev->id }}">
                                    {{ $dev->itemType->item_name  }} -
                                      {{ $dev->serial_no }}
                                       <!-- {{ $dev->oemRelation->oem_name }} -  -->
                                      <!-- {{ $dev->categoryRelation->category_name  }}-  -->
                                       
                                  </option>
                                    @endforeach
                                @else
                                  <option value=''>--No Device for allocation--</option>
                                @endif
                            </select>
                            <input type="hidden" name="device_hidden" id="device_hidden">
                            <span id="error-rate" class="error-message"></span>
                        </div>

                        <div class="col-md-6">
                          <label for="issued_on" class="form-label"><b>Issued On</b></label>
                          <input type="text" class="form-control" id="issued_on" name="issued_on" maxlength="10" >
                        </div>

                        <div class="col-md-6">
                          <label for="returned_on" class="form-label"><b>Returned On</b></label>
                          <input type="text" class="form-control" id="returned_on" name="returned_on" maxlength="10" >
                        </div>

                        <div class="col-md-6">
                            <label for="assigned_to" class="form-label"><b> Assigned To</b></label>
                            <select name="assigned_to" id="assigned_to" class="form-control">
                                <option value=''>--Assigned to --</option>
                                @foreach($assigned as $assign)
                                <option value="{{ $assign->id }}">{{ $assign->assigned_to }}  </option>
                                @endforeach
                            </select>
                            <input type="hidden" name="assigned_to_hidden" id="assigned_to_hidden">
                            <span id="error-rate" class="error-message"></span>
                        </div>
                  <br>
...
<script src="{{asset('assets/js/modules/admin/allocation.js')}}"></script>
@endsection